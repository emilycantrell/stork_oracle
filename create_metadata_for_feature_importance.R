...
feature_importance <- fread(feature_importance_path) %>%
  slice_sample(prop = 1)
n_feature_importance <- nrow(feature_importance)
feature_importance <- feature_importance %>%
  mutate(i = 1:n_feature_importance) %>%
  arrange(desc(importance), i)
increment <- ceiling(n_feature_importance / 10)

metadata <- metadata %>%
  mutate(feature_set_decile10 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 0 * increment], 1, 0),
         feature_set_decile9 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 1 * increment], 1, 0),
         feature_set_decile8 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 2 * increment], 1, 0),
         feature_set_decile7 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 3 * increment], 1, 0),
         feature_set_decile6 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 4 * increment], 1, 0),
         feature_set_decile5 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 5 * increment], 1, 0),
         feature_set_decile4 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 6 * increment], 1, 0),
         feature_set_decile3 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 7 * increment], 1, 0),
         feature_set_decile2 = ifelse(variable_name %in% feature_importance$feature_importance_names[1:increment + 8 * increment], 1, 0),
         feature_set_decile1 = ifelse(variable_name %in% feature_importance$feature_importance_names[(9 * increment + 1):n_feature_importance], 1, 0))

add_top_feature <- function(feature) {
  metadata <<- metadata %>%
    mutate(new_feature_set = ifelse(variable_name == feature_importance$feature_importance_names[feature], 1, 0))
  colnames(metadata)[colnames(metadata) == "new_feature_set"] <<- paste0("feature", feature)
}
walk(1:increment, add_top_feature)