...
# This file creates a file that assigns data type to columns and indicate which
# feature set component each feature belongs to (right now, there's one feature 
# set component for each CBS dataset, for PreFer official train, and for 
# sex_and_gender). Feature sets are meant to facilitate manual feature
# selection, but for now we don't do feature selection and always use all
# features from persoontab, householdbus, and train.csv.

# This function creates a dataframe indicating variable times for each feature
...
continuous_features <- prefer_official %>%
...
categorical_features <- prefer_official %>%
...
variable_type_metadata_prefer_official_train <- record_variable_types_in_df(cbs_data_file = prefer_official)

# Add more information about feature sets, using the codebook for train.csv. Which CBS datasets do the variables originally come from?
...
metadata_gbapersoontab <- variable_type_metadata_gbapersoontab %>%
  mutate(feature_set_GBAPERSOONTAB = 1) # Indicate feature set component
...
metadata_gbahuishoudensbus <- variable_type_metadata_gbahuishoudensbus %>%
  mutate(feature_set_GBAHUISHOUDENSBUS = 1) # Indicate feature set component
