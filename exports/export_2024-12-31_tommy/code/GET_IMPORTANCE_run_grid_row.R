  ...
  get_model_for_catboost <- function(learning_rate, subsample, depth) {
    if (is.null(selection_set)) {
      ...
      model_fit <<- catboost.train(training_set_data, params = list(
        ...
        subsample = subsample,
        depth = depth,
        #thread_count = n_thread_within_worker, # This line is commented out because it caused a bug, 2024-10-28
        logging_level = "Silent"
      ))
      feature_importance <- catboost.get_feature_importance(model = model_fit)
      feature_importance_names <- rownames(feature_importance)
      rownames(feature_importance) <- NULL
      feature_importance <- cbind(feature_importance_names, feature_importance) %>%
        as_tibble() %>%
        rename(importance = V2) %>%
        mutate(importance = as.numeric(importance)) %>%
        write.csv(paste0(model_path, "feature_importance"))
