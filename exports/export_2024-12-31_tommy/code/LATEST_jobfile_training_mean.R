...
parallelize_evaluation_sets_instead_of_grid_rows_in_second_round <- TRUE
...
# Modeling options
model_settings <- list(
  training_mean = list(tibble(grid_row = pmap(expand_grid(
    hyperparameter = c(NA)
  ),
  list)),
  steps = c(1)) #,
...
results_path <- "results_training_mean_2024-12-26"