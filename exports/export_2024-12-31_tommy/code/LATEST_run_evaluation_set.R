...
run_evaluation_set <- function(evaluation_set, sampling_file_content, first_round_evaluation_sets, evaluation_sets_predictions, evaluation_sets_rinpersoon, selection_set, bootstrap_samples, training_set_outcome_mean, metrics_for_all_pipelines, metrics_for_winning_pipelines, threshold_reference, save_only_winning_hyperparameter_draw_results) {
  print(Sys.getpid())

  evaluation_set_rinpersoon <- get_rinpersoon(evaluation_set, sampling_file_content)
  ...
  bootstrap_samples_output <- map(bootstrap_samples, ~run_bootstrap_sample(.x, evaluation_set_predictions, evaluation_set_outcomes, bootstrapping_indices, n_evaluation_set, training_set_outcome_mean, metrics, check_threshold_reference_if_applicable, threshold_reference)) %>%
    list_rbind() %>%
