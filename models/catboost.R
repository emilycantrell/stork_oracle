# On CBS server, this is file version catboost_2024-08-28.R
...
# This file fits a catboost model
...
# Function to generate pool object which can be fed into catboost
generate_pool_for_catboost <- function(feature_data, outcome_data) { 
  # Clean data
  ...
  features_and_outcome <- left_join(cleaned_feature_data, outcome_data, by = "RINPERSOON")
  # Separate features and outcome, and remove ID variables
  features_for_pool <- features_and_outcome %>%
    select(-any_of(c("RINPERSOON", "RINPERSOONS", "outcome")))
  outcome_for_pool <- features_and_outcome %>%
    pull(outcome)
  # Create pool 
  pool <- catboost.load_pool(data = features_for_pool, 
                             label = outcome_for_pool)
  return(pool)
  }
  
# Generate the pools
train_pool <- generate_pool_for_catboost(features_stork_oracle_train, outcome_data)
validation_pool <- generate_pool_for_catboost(features_stork_oracle_validation, outcome_data)
test_pool <- generate_pool_for_catboost(features_stork_oracle_test, outcome_data)
...
for(i in 1:nrow(grid)) {
  ...
  evaluation_scores <- catboost.eval_metrics(model = model,
                                             pool = validation_pool,
                                             metrics = "Logloss")
  tuning_results$score[i] <- as_tibble(evaluation_scores$Logloss)
  tuning_results$step[i] <- as_tibble(1:length(evaluation_scores$Logloss))
}
...
best_params <- list(
  ...
  rsm = best_hyperparameters$rsm
)

best_model <- catboost.train(learn_pool = train_pool,
                             test_pool = validation_pool, 
                             params = best_params)
best_model$step <- best_hyperparameters$step

# Save model
model_path <- "H:/pmt/stork_oracle/4b_catboost/untuned_catboost_with_prefer_official_train_2024-08-26"
...
pred_probabilities_validation <- catboost.predict(best_model, validation_pool, prediction_type = "Probability", ntree_end = best_model$step)
...
for(threshold in thresholds) { 
  ...
  f1 <- F1_Score(y_pred = pred_labels_validation, y_true = observed_outcomes_validation, positive = "1")
  if(is.na(f1)) {
    break
  }
  if(f1 > best_f1) { 
...
pred_probabilities_test <- catboost.predict(best_model, test_pool, prediction_type = "Probability", ntree_end = best_model$step)
...
best_model_shrunken <- catboost.shrink(best_model, best_model$step, 0)
feature_importances <- catboost.get_feature_importance(best_model, pool = train_pool)
feature_importances <- as_tibble(feature_importances) %>%
  mutate(feature = rownames(feature_importances)) %>%
  left_join(prefer_official_train_codebook) %>%
  group_by(Source) %>%
  summarise(importance = sum(V1))

importance_path <- "H:/pmt/stork_oracle/4b_catboost/untuned_catboost_with_prefer_official_train_importance_2024-08-26"
